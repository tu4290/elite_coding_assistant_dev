...
class DocumentSource(BaseModel):
    ...
    def check_source_provided(cls, v: Optional[str], values):
        data = values.data  # Access all fields via .data
        if (
            not data.get("file_path")
            and not data.get("url")
            and not data.get("content_bytes")
        ):
            raise ValueError(
                "Either file_path, url, or content_bytes must be provided."
            )
        if data.get("content_bytes") and not v:
            raise ValueError(
                "file_name must be provided if content_bytes is used."
            )
        return v
...
class ContentChunk(BaseModel):
    ...
    )  # e.g., page number, section header
...
class ProcessingStatus(str, Enum):
    ...
    SUCCESS = "success"
    PARTIAL_SUCCESS = (
        "partial_success"  # e.g. content extracted, but embedding failed
    )
    FAILURE = "failure"
...
class ProcessingResult(BaseModel):
    ...
    chunk_count: int = 0 # Default to 0 to avoid Optional
    embedded_chunk_count: int = 0 # Default to 0
...
class ProcessedDocumentEntry(BaseModel):
    ...
    document_type: DocumentType
    status: ProcessingStatus
    processing_started_at: datetime.datetime = Field(
...
class KnowledgeBaseEntry(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict
    )  # Chunk-specific metadata (page, section)
    document_metadata: Dict[str, Any] = Field(
        default_factory=dict
    )  # Metadata of the parent document
